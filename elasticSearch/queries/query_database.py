...
def get_all_docs_in_db(elastic_search_client: Elasticsearch, src_includes: list = SRC_INCLUDES, all: bool= True, from_n: int=0) -> list:
    '''
    :param elastic_search_client: Elasticsearch client
    ...
    results = elastic_search_client.search(
        index='bahamas',
        from_=(from_n),
        size=get_number_docs_in_db(elastic_search_client) if all else 10000,
        query={'match_all' : {} },   
        source_includes=src_includes)['hits']['hits']
    return convert_hits(results)
...
def get_knn_res(doc_to_search_for:str, query_type:str, elastic_search_client:Elasticsearch, n_results:int):
    ...
    :param query_type: type of the query to be searched for; must be one of:
        "doc2vec", "sim_docs_tfidf", "google_univ_sent_encoding", "huggingface_sent_transformer", "inferSent_AE", 
        "pca_optics_cluster", "argmax_pca_cluster"
    :param elastic_search_client: Elasticsearch client
