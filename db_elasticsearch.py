...
def init_db(client: Elasticsearch, num_dimensions: int, sim_docs_vocab_size: int, find_doc_vocab_size: int):
    '''
...
def insert_documents(src_paths: list, model: Doc2Vec, client: Elasticsearch, google_model, huggingface_model, sim_doc_tfidf_vectorization: np.ndarray, find_doc_tfidf_vectorization: np.ndarray, image_path: str = None):
    '''
...
if __name__ == '__main__':
    ...
    if ('bahamas' not in client.indices.get_alias(index='*')) or (client.indices.get_mapping(index='bahamas')['bahamas']['mappings']['properties']['embedding']['dims'] != NUM_DIMENSIONS):
        ...
        init_db(client, num_dimensions=NUM_DIMENSIONS, sim_docs_vocab_size=sim_docs_vocab_size, find_doc_vocab_size=find_doc_vocab_size)
    ...
    insert_documents(src_paths, model, client, image_path=image_src_path, google_model=google_model, huggingface_model=huggingface_model, sim_doc_tfidf_vectorization=sim_docs_D, find_doc_tfidf_vectorization=find_doc_D)  
